...
class PoissonDiskSampler extends Sampler {
  ...
  set rng(Random rng) {
    us = UniformSampler.withRng(_rect, rng);
    rng = rng;
  }
  ...
  List<Vector2> _generatePoints(double r, int k) {
    ...
    List<List<Vector2>> grid = List();
    for (int i = 0; i < boxCols; i++) grid.add(List(boxRows));

    // active/processing points
    List<Vector2> active = List();
    ...
    while (active.isNotEmpty) {
      ...
      for (i = 0; i < k; i++) {
        ...
        bool isValidSpace(Vector2 candidate, int gridX, int gridY) {
          ...
          for (int i = max(0, gridX - 1); i < min(gridX + 2, boxCols); i++) {
            for (int j = max(0, gridY - 1); j < min(gridY + 2, boxRows); j++) {
              if (grid[i][j] != null && grid[i][j].distanceTo(candidate) < r)
                return false;
            }
          }
          return true;
        ...
        bool valid = gridX >= 0 &&
            ...
            gridY >= 0 &&
            gridY < boxRows &&
            isValidSpace(candidate, gridX, gridY);
  ...
  List<Vector2> generatePoints(num numPoints, [k = 30]) {
    ...
    return _generatePoints(
        sqrt((_rect.width * _rect.height) / numPoints) * 0.82 + 150 / numPoints,
        k);
  }
}
